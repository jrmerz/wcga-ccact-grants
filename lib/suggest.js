...
var db = global.db;
var collection;
var searchCollection = app.collection;
...
app.get('/rest/mySuggestion', function(req, resp){
...
app.get('/rest/removeSuggestion', function(req, resp){
    ...
    collection.findOne({id: _id}, {suggestedBy: 1}, function(err, result){
        if( err ) return resp.send({error: true, message: err});
        ...
        collection.remove({id: _id}, function(err, result){
            if( err ) return resp.send({error: true, message: err});
...
function removeGrant(id, callback) {
    searchCollection.remove({_id: id}, function(err, result){
        if( err ) return callback({error: true, message: err});
        callback(null, result);
    });
}
