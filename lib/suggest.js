...
var bubble = require('../import/lib/bubble');
...
app.get('/rest/mySuggestions', function(req, resp){
    ...
    collection
        .find({suggestedBy: req.user.id}, {history: 0})
        .sort({title: 1})
...
app.post('/rest/suggest', function(req, resp){
    ...
    data.suggestedBy = req.user.id;
    if( req.user.email ) data.suggestedByEmail = req.user.email;
    data.suggestedByName = req.user.displayName;
    data.suggestedByAuth = req.user.auth;
    data.status = 'pending';
    data.history = [];
...
app.get('/rest/removeSuggestion', function(req, resp){
    ...
    collection.findOne({_id: new ObjectID(id)}, {suggestedBy: 1, suggestedByAuth: 1}, function(err, result){
        if( err ) return resp.send({error: true, message: err});
        if( !result ) return resp.send({error: true, message: 'grant not found'});

        if( result.suggestedBy != req.user.id || result.suggestedByAuth != req.user.auth ) {
            return resp.send({error: true, message: 'you did not suggest this grant'});
        }
...
function approveGrant(id, callback) {
    collection.findOne({_id: new ObjectID(id)}, function(err, result){
        ...
        delete result.suggestedName;
        delete result.suggestedEmail;
        delete result.suggestedAuth;

        result.id = id;

        bubble.process(result);

        controlledVocab.process(result);
